...
SelectMenuToolNode::SelectMenuToolNode(std::vector<std::shared_ptr<SelectToolNode>> toolNodes, std::wstring text, GLuint image) noexcept
    :toolNodes(toolNodes), text(text), image(image){
}

void SelectMenuToolNode::OnSelect() noexcept{}
const GLuint SelectMenuToolNode::GetImage() const noexcept{return image;}
